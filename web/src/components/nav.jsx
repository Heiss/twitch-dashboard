...
function Link({ to, isActive, children, icon }) {
    return (
        <li class="mr-6 my-2 md:my-0">
            <a href={to} class={`block py-1 md:py-3 pl-1 align-middle ${isActive ? 'text-blue-400' : 'text-gray-500'} no-underline hover:text-gray-100 border-b-2 ${isActive ? 'border-blue-400' : 'border-gray-900'} hover:border-blue-400`}>
                <i class={`${icon} mr-3 ${isActive ? 'text-blue-400' : ''}`}></i><span class="pb-1 md:pb-0 text-sm">{children}</span>
            </a>
        </li >
    );
}
...
function UserMenu() {
    return (<div id="userMenu" class="bg-gray-900 rounded shadow-md mt-2 absolute mt-12 top-0 right-0 min-w-full overflow-auto z-30 invisible">
        <ul class="list-reset">
    ...
    if (await user.is_logged()) {
        return <Fragment>
            <button id="userButton" class="flex items-center focus:outline-none mr-3">
                <img class="w-8 h-8 rounded-full mr-4" src="http://i.pravatar.cc/300" alt="Avatar of User" /> <span class="hidden md:inline-block text-gray-100">Hi, {logged_username}</span>
            </button>
            <UserMenu />
        </Fragment>;
    } else {
        return <a href="https://id.twitch.tv/oauth2/authorize" class="px-4 py-2 block text-gray-100 hover:bg-gray-800 no-underline hover:no-underline">Login</a>;
    }
